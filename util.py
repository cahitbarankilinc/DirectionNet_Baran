...
def von_mises_fisher(mean, concentration, shape):
  ...
  with tf.name_scope(None, 'von_mises_fisher', [mean, concentration, shape]):
    ...
    # --- DİNAMİK ŞEKİLLER ---
    dyn = tf.shape(mean)                          # [B, N, 3]
    batch    = tf.cast(dyn[0], tf.int32)
    channels = tf.cast(dyn[1], tf.int32)
    ...
    # multiples: int32 tensor olmalı; Dimension(None) KULLANMA!
    cartesian = tf.tile(
        cartesian[tf.newaxis, tf.newaxis, :],
        tf.stack([batch, channels,
                  tf.constant(1, tf.int32),
                  tf.constant(1, tf.int32),
                  tf.constant(1, tf.int32)], axis=0))

    mean = tf.tile(
        mean[:, :, tf.newaxis, tf.newaxis],
        tf.stack([tf.constant(1, tf.int32),
                  tf.constant(1, tf.int32),
                  height, width,
                  tf.constant(1, tf.int32)], axis=0))
