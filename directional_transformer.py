...
class RMSNormalization(keras.layers.Layer):
  """RMSNorm implementation mirroring T5-style normalization."""
...
class DropPath(keras.layers.Layer):
  """Stochastic depth following Identity mapping at inference time."""
  ...
  def call(self, inputs, training=None):
    ...
    # Broadcast along non-batch dimensions for per-example dropping.
...
class _TransformerBlock(keras.layers.Layer):
  ...
  def __init__(self,
    ...
    self.attention_dropout_rate = (
        dropout_rate if attention_dropout_rate is None else attention_dropout_rate)
    self.mlp_dropout_rate = (
        dropout_rate if mlp_dropout_rate is None else mlp_dropout_rate)

    self.norm1 = self._make_norm(norm_type, norm_epsilon)
    ...
    self.attention_dropout = keras.layers.Dropout(self.attention_dropout_rate)
    self.attention_output_dropout = keras.layers.Dropout(
        self.attention_dropout_rate)
    ...
    self.norm2 = self._make_norm(norm_type, norm_epsilon)
    self.mlp_dense1 = keras.layers.Dense(mlp_dim, activation=_approximate_gelu)
    self.mlp_dropout1 = keras.layers.Dropout(self.mlp_dropout_rate)
    self.mlp_dense2 = keras.layers.Dense(hidden_size)
    self.mlp_dropout2 = keras.layers.Dropout(self.mlp_dropout_rate)
    self.attention_droppath = DropPath(rate=droppath_rate)
    self.mlp_droppath = DropPath(rate=droppath_rate)

  def _make_norm(self, norm_type, epsilon):
  ...
  def call(self, token_features, *, training, split_heads_fn, merge_heads_fn):
    ...
    attention_output = self.attention_output_dropout(attention_output,
                                                     training=training)
    attention_output = self.attention_droppath(attention_output,
                                               training=training)
    ...
    mlp_output = self.mlp_droppath(mlp_output, training=training)
