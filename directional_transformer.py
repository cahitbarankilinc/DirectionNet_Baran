...
class DirectionalContextTransformer(keras.Model):
  ...
  def __init__(self,
    ...
    self.positional_embedding = self.add_weight(
        ...
        shape=[1, 4 + self.context_token_count, hidden_size],
  ...
  def call(self, expectation, context_embedding, training=False):
    ...
    context_tokens = tf.reshape(context_tokens,
                                [-1, self.context_token_count, self.hidden_size])
    ...
    refined = self.output_projection(token_features[:, self.context_token_count:, :])
